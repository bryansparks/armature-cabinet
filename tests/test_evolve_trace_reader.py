...
def _seed_latency_cost(db: Path, rows):
    con = sqlite3.connect(db)
    con.execute("""CREATE TABLE traces (
        id INTEGER PRIMARY KEY AUTOINCREMENT, run_id TEXT, workflow_name TEXT,
        stage_id TEXT, role_type TEXT, model TEXT, input_tokens INTEGER DEFAULT 0,
        ...
        agent_id TEXT, agent_version TEXT, active_skill_ids_json TEXT DEFAULT '[]')""")
    con.executemany("""INSERT INTO traces (run_id, workflow_name, stage_id, role_type, model,
        input_tokens, output_tokens, latency_ms, success, output_valid, timestamp,
...
def test_read_summary_emits_high_latency_above_threshold(tmp_path: Path):
    db = tmp_path / "traces.db"
    rows = [
        ("r", "wf", "s", "worker", "m", 0, 0, 4000, 1, 1, "t", "a", "0.1.0", "[]"),
        ("r", "wf", "s", "worker", "m", 0, 0, 3500, 1, 1, "t", "a", "0.1.0", "[]"),
        ("r", "wf", "s", "worker", "m", 0, 0, 1000, 1, 1, "t", "a", "0.1.0", "[]"),
    ]
    _seed_latency_cost(db, rows)
    s = read_summary(db, agent_id="a", agent_version="0.1.0", skill_tools={},
                     min_traces=1, latency_threshold_ms=3000)
    syms = dict(s.dominant_symptoms)
    assert syms.get("HIGH_LATENCY") == 2  # two rows above 3000


def test_read_summary_emits_high_cost_above_threshold(tmp_path: Path):
    db = tmp_path / "traces.db"
    rows = [
        ("r", "wf", "s", "worker", "m", 4000, 5000, 100, 1, 1, "t", "a", "0.1.0", "[]"),
        ("r", "wf", "s", "worker", "m", 100, 100, 100, 1, 1, "t", "a", "0.1.0", "[]"),
    ]
    _seed_latency_cost(db, rows)
    s = read_summary(db, agent_id="a", agent_version="0.1.0", skill_tools={},
                     min_traces=1, cost_threshold_tokens=8000)
    syms = dict(s.dominant_symptoms)
    assert syms.get("HIGH_COST") == 1  # one row above 8000 total tokens


def test_read_summary_default_thresholds_do_not_emit(tmp_path: Path):
    db = tmp_path / "traces.db"
    rows = [("r", "wf", "s", "worker", "m", 10, 10, 100, 1, 1, "t", "a", "0.1.0", "[]")]
    _seed_latency_cost(db, rows)
    s = read_summary(db, agent_id="a", agent_version="0.1.0", skill_tools={}, min_traces=1)
    assert "HIGH_LATENCY" not in dict(s.dominant_symptoms)
    assert "HIGH_COST" not in dict(s.dominant_symptoms)
